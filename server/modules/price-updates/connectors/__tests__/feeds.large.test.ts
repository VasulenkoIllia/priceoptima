...
//   PO_FEEDS_DIR=/шлях/до/вигрузок npx vitest run server/modules/price-updates/connectors/__tests__/feeds.large.test.ts --reporter=verbose
...
const FEEDS: { file: string; connector: FeedConnector; options?: Partial<AdapterOptions> }[] = [
  { file: 'sandi.json', connector: 'sandi' },
  { file: 'sanwell.xml', connector: 'sanwell' },
  // у YML САНДІ ціна роздрібна
  { file: 'sandi.yml', connector: 'yml', options: { hasPurchasePrice: false } },
];
...
describe.skipIf(!FEEDS_DIR)('повні вигрузки постачальників', () => {
  for (const feed of FEEDS) {
    ...
    it.skipIf(!existsSync(path))(`${feed.file}`, { timeout: 60_000 }, () => {
      ...
      const result = parseFeed(feed.connector, body, feed.options);
