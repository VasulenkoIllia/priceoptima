...
export const TEMPLATE_COLUMNS: readonly TemplateColumn[] = [
  ...
  { header: 'РРЦ з ПДВ', width: 14, note: 'Рекомендована роздрібна ціна з ПДВ. Залиште порожньою, якщо РРЦ не встановлюєте.' },
  { header: 'Наявність', width: 14, note: 'Кількість («12», «100+») або словом: «є», «мало», «немає», «під замовлення».' },
  { header: 'Кратність', width: 12, note: 'Мінімальний крок замовлення в одиницях (напр., 6, якщо продається упаковками по 6).' },
...
const INSTRUCTIONS_INTRO: [string, string][] = [
  ...
  ['', ''],
  ['Аркуш «Прайс»', 'Перший рядок містить заголовки, не змінюйте їх і не додавайте рядків над ними. Дані починаються з другого рядка, по одному товару в рядку.'],
  ...
  ['Наявність', 'Читаємо число («12», «100+» означає 100 і більше) або слово: «є», «мало», «немає», «під замовлення». Порожня клітинка означає, що наявність невідома.'],
  ['Зайві колонки', 'Можна лишати свої додаткові колонки: ми зіставляємо їх за назвою заголовка й непотрібні пропускаємо.'],
  ['Формат файлу', 'xlsx, старий xls або csv.'],
  ['', ''],
  ['Колонки', ''],
];
